...
function generateEvents(filterSeed: number, selectedBrand: string, selectedSku: string, selectedRetailer: string, selectedMechanic: string): PromoEvent[] {
  ...
  let id = 0
  let dietCokeGoodPerformerCount = 0 // Track to limit to 2 good performers for Diet Coke
  
  for (const sku of skus) {
    const brand = skuBrandMap[sku] || "CC Classic"
    ...
    for (const ret of retailers) {
      for (const mech of mechanics) {
        ...
        // STORYLINE: Diet Coke has mostly BAD promo performance
        ...
        const isDietCoke = brand === "Diet Coke"
...
// ---------- AI Insights - STORYLINE: Highlight Diet Coke bad performers ----------
const aiInsights = [
  { type: "negative" as const, text: "Diet Coke SKUs dominate the Lose-Lose quadrant (bottom-left): 18 of 24 events are Diet Coke with long promo duration (4+ weeks) and shallow price cuts (<15%). These are destroying value.", highlight: true },
  { type: "negative" as const, text: "Diet Coke 500ml and 1.5L show worst Trade KPI (-180 to -220) with average Client KPI. Long promo windows causing forward buying without incremental lift.", highlight: true },
  { type: "warning" as const, text: "Compare with CC Zero in Win-Win quadrant: shorter durations (2 weeks), deeper cuts (25-30%), and display support driving both Trade and Client KPI positive." },
  { type: "warning" as const, text: "Recommendation: Navigate to Performance by Lever to understand what promo mechanics and depths drive success, then optimize Diet Coke promos accordingly." },
]
